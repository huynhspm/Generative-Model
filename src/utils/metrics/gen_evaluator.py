...
if __name__ == "__main__":
    evaluator = Gen_Evaluator(device=torch.device('cuda'))

    real = torch.randn(50, 3, 64, 64)
    fake = torch.randn(50, 3, 64, 64)
    
    evaluator.fill_fake_img_batch(fake, return_results=False)
    evaluator.fill_real_img_batch(real)

    print(evaluator.compute_IS())
    print(evaluator.compute_FID())